...
class RandomWalkWithRestart:
    ...
    def create_random_walks(self, seeds_in=False, all=False):
       ...
       # print(f'TEST JOIN {len(list(set(self.removed).intersection(self.G.nodes())))}')
        if seeds is not None:
            ...
            walks = walker.random_walks(self.G_conv, n_walks=10, alpha=0.1,
                                        walk_len=15, start_nodes=converted_seeds, q=0.1, p=0.5)
        else:
    ...
    def get_neighbours_vector(self, sel_cores, sel_keys, sel_top):
        ...
        for i, seed in enumerate(self.seeds):
            ...
            if len(ret_net_tops[i]) < self.args.n_top_hubs and len(ret_tops[i]) > 0:
...
#         # print(seeds)
         ...
         walks = walker_obj.create_random_walks(seeds_in=pd.read_csv(f'datasets/mes/split_transductive/train/pubdataedges_test_trans_kcore_2.csv')['source'].tolist() + pd.read_csv(f'datasets/mes/split_transductive/train/pubdataedges_test_trans_kcore_2.csv')['target'].tolist())
